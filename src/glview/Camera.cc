...
void Camera::setup(std::vector<double> params)
{
  if (params.size() == 7) {
    ...
    object_rot.z() = (!dir[1] && !dir[0]) ? dir[2] < 0 ? 0 : 180 : -atan2_degrees(dir[1], dir[0]) + 90;
    object_rot.y() = 0;
...
void Camera::updateView(const std::shared_ptr<const FileContext>& context, bool enableWarning)
{
  ...
  if (vpr) {
    if (vpr->getVec3(x, y, z, 0.0)) {
      ...
      LOG(message_group::Warning, "Unable to convert $vpr=%1$s to a vec3 or vec2 of numbers",
          vpr->toEchoString());
    }
  }
  ...
  if (vpt) {
    if (vpt->getVec3(x, y, z, 0.0)) {
      ...
      LOG(message_group::Warning, "Unable to convert $vpt=%1$s to a vec3 or vec2 of numbers",
          vpt->toEchoString());
    }
  }
...
std::string Camera::statusText() const
{
  ...
  boost::format fmt(
    _("Viewport: translate = [ %.2f %.2f %.2f ], rotate = [ %.2f %.2f %.2f ], distance = %.2f, fov = "
      "%.2f"));
