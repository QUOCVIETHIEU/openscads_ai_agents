...
std::string buildPromptFromHistory(const std::vector<AIChatMessage>& history, bool use_mcp)
{
  ...
  if (use_mcp) {
    oss << "Native MCP tools are available — USE THEM:\n"
           ...
           "- set_editor_code(code): apply the FULL OpenSCAD script to the AI target file "
           "and run F5 CSG preview.\n"
           "- trigger_preview(): re-run F5 preview without changing code.\n"
           "- trigger_render() / trigger_build(): full F6 mesh — call ONCE when design is final.\n"
           "- list_project_files / read_project_file(path) / get_project_rules: inspect the open "
           "project (assets, rules, design files).\n"
           "- get_model_info(): bbox/errors after preview or render.\n"
           ...
           "Workflow: get_skill → set_editor_code (F5) → get_model_info → view_all → "
           "get_preview_image → iterate with set_editor_code/trigger_preview → when right, "
           "trigger_render once (F6).\n"
