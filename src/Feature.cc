...
const Feature Feature::ExperimentalObjectFunction(
  "object-function", "Enable object function to allow user creation of objects.");
...
const Feature Feature::ExperimentalAiFeatures(
  "ai-features", "Enable AI assistant for generating OpenSCAD code via configured API endpoints.");
...
void ExperimentalFeatureException::check(const Feature& feature)
{
  if (!feature.is_enabled()) {
    throw ExperimentalFeatureException(STR("ERROR: Experimental feature not enabled: '",
